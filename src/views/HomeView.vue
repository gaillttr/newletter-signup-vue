<script>
import ListFeatureVue from '../component/ListFeature.vue'
...
<template>
  <body class=" flex flex-col dsk:justify-center items-center dsk:h-screen dsk:bg-neo-charcoal-grey">
        ...
        <div class=" flex flex-col gap-6 w-[327px] dsk:w-full">
          <div class=" flex flex-col gap-2">
            <div class=" flex justify-between text-[12px] leading-[18px] font-bold">
              ...
              <h5 v-show="successForm == false" class=" text-red-600 ">Valid email required</h5>
            </div>
            <input @keydown="successForm = null" class=" h-14 px-6 border rounded-lg" :class="`${inputState}`"
              type="email" placeholder="email@gmail.com" v-model="emailValue">
          </div>
          <button @click="signUpEmail()" class=" h-14 bg-emerald-600 text-white font-bold rounded-xl shadow-xl"
    ...
    <main v-show="successForm"
      class=" flex flex-col gap-[263px] gap-10 w-[375px] dsk:w-[504px] dsk:h-[520px] pt-[149px] dsk:pt-12 pb-10 dsk:pb-16 px-6 dsk:px-16 bg-neo-white font-roboto dsk:rounded-[36px]">
      <div class=" flex flex-col items-start gap-10 ">
        <img :src="iconSuccess" alt="iconSuccess">
        <div class=" flex flex-col gap-6 text-neo-charcoal-grey">
          <h1 class=" text-[40px] dsk:text-[56px] leading-[40px] dsk:leading-[56px] font-bold">Thanks for
            subscribing!</h1>
          <p>
      ...
      <button @click="signOutEmail()" class=" w-full h-14 bg-red-400 text-white font-bold rounded-xl shadow-xl"
